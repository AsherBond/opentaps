...
if [ ! -f "../derby-ds.xml" ]; then
  cp ${ofbizHome}/framework/appserver/templates/jboss510/patches/derby*.xml ..
  echo "derby datasource configuration installed"
fi

# configure the jboss entity engine (patch) configuration
if [ ! -f "${ofbizHome}/framework/entity/config/entityengine-jboss510.xml" ]; then
  patch -i ${ofbizHome}/framework/appserver/templates/jboss510/patches/jboss-ee-cfg.patch -o ${ofbizHome}/framework/entity/config/entityengine-jboss510.xml ${ofbizHome}/framework/entity/config/entityengine.xml
...
if [ -f "${ofbizHome}/framework/entity/config/entityengine-jboss510.xml" ]; then
  ...
  mv ${ofbizHome}/framework/entity/config/entityengine-jboss510.xml ${ofbizHome}/framework/entity/config/entityengine.xml
...
if [ -f "${ofbizHome}/framework/entity/config/entityengine.xml.jbak" ]; then
  mv ${ofbizHome}/framework/entity/config/entityengine.xml ${ofbizHome}/framework/entity/config/entityengine-jboss510.xml
...
if [ ! -f "../../../../bin/run.conf.obak" ]; then
  ...
  cp ${ofbizHome}/setup/jboss510/run.conf ../../../../bin/run.conf
