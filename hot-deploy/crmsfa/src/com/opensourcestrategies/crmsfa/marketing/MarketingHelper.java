...
public class MarketingHelper {                                                     
    ...
    public static final String module = MarketingHelper.class.getName();
    ...
    public static long countContactListMembers(String contactListId, EntityCondition condition, GenericDelegator delegator) throws GenericEntityException {
        ...
        if (value == null || value.get("partyId") == null) return 0;
    ...
    public static long countContactListMembersByCountry(String contactListId, String countryGeoId, GenericDelegator delegator) throws GenericEntityException {
        EntityCondition condition = new EntityExpr("countryGeoId", EntityOperator.EQUALS, countryGeoId);
    ...
    public static Map getZipToBMCCode(GenericDelegator delegator) throws GenericEntityException {
        Map results = FastMap.newInstance();
        List values = delegator.findAllCache("UspsZipToBmcCode");
        for (Iterator iter = values.iterator(); iter.hasNext(); ) {
            GenericValue value = (GenericValue) iter.next();
            String bmcCode = value.getString("bmcCode");
            if (bmcCode == null || bmcCode.trim().length() == 0) continue;
     ...
     */
    public static String findBMCCode(String zip3String, Map overrideMap) {
        String bmcOverride = (String) overrideMap.get(zip3String);
        if (bmcOverride != null) return bmcOverride;
