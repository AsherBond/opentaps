...
public class WarehouseSecurity extends OpentapsSecurity {
    ...
    public static final String module = WarehouseSecurity.class.getName();
    ...
    public boolean hasFacilityPermission(String facilityId, GenericValue userLogin, String permissionId) {
        ...
        if (UtilValidate.isEmpty(userLogin) || UtilValidate.isEmpty(permissionId)) return false;
        ...
        try {
            ...
            List facilityPartySecPermConditions = new ArrayList();
            if (UtilValidate.isNotEmpty(facilityId)) {
                
                // If facilityId is not specified, check to see if the user has permission for ANY facility
                facilityPartySecPermConditions.add(new EntityExpr("facilityId", EntityOperator.EQUALS, facilityId));
            }
            facilityPartySecPermConditions.add(new EntityExpr("partyId", EntityOperator.EQUALS, userLogin.get("partyId")));
            facilityPartySecPermConditions.add(new EntityExpr("permissionId", EntityOperator.EQUALS, permissionId));
            facilityPartySecPermConditions.add(EntityUtil.getFilterByDateExpr());

            List facilityPartySecPerms = delegator.findByAnd("FacilityPartySecurityPermission", facilityPartySecPermConditions);
            if (UtilValidate.isNotEmpty(facilityPartySecPerms)) return true;
     ...
     */
    public List getUserFacilities(GenericValue userLogin) {
        ...
        try {
            if (getSecurity().hasPermission("WRHS_ADMIN", getUserLogin())) {
                ...
                List conditions = UtilMisc.toList(new EntityExpr("partyId", EntityOperator.EQUALS, userLogin.get("partyId")), EntityUtil.getFilterByDateExpr());
