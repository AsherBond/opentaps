   ...
   <#if !(inventoryItem?exists)>
    ...
    <#else>
       <#if !(inventoryTransfer?exists)>
        ...
        <#else>
            <form method="post" action="<@ofbizUrl>CheckInventoryForUpdateTransfer</@ofbizUrl>" name="transferform" style="margin: 0;">
            <input type="hidden" name="inventoryTransferId" value="${inventoryTransferId?if_exists}">
        </#if>

        <script type="text/javascript">
            function setNow(field) { eval('document.transferform.' + field + '.value="${getLocalizedDate(Static["org.ofbiz.base.util.UtilDateTime"].nowTimestamp())}"'); }
        </script>
        ...
        <select name="statusId" class="selectBox">
            ...
            <#list statusItems as statusItem>
              ...
              <#if ! ((! inventoryTransfer?exists) && statusItem.statusId == "IXF_COMPLETE")>
                <option value="${(statusItem.statusId)?if_exists}" ${(curStatusId == statusItem.statusId)?string("selected=\"selected\"", "")}>${(statusItem.get("description",locale))?if_exists}</option>
              </#if>
            </#list>
        </select>
        </td>
    ...
    <tr>
       <td width="14%">&nbsp;</td>
        ...
        <td width="6%">&nbsp;</td>
        <td width="74%">
        <input type="text" name="sendDate" value="${getLocalizedDate(inventoryTransfer?default({}).sendDate?if_exists)}" size="22" class="inputBox">
    ...
    <#else>
        <tr>
            ...
            <td width="6%">&nbsp;</td>
            <td width="74%">
            <input type="text" name="receiveDate" value="${getLocalizedDate(inventoryTransfer.receiveDate)}" size="22" class="inputBox">
    ...
    <tr>
        <td colspan="3">&nbsp;</td>
        <#if !(inventoryTransfer?exists)>
            <td align="left"><input type="submit" value="${uiLabelMap.ProductTransfer}"></td>
        <#else>
            <td align="left">
                <#if parameters.forceComplete?default("false") == "true">
                  ...
                  <@inputConfirm title=uiLabelMap.WarehouseForceComplete form="transferform" />
                <#else>
