    ...
    <#if orderHeader?has_content>
       ...
       <fo:table-header>
           <fo:table-row>
            ...
            <fo:table-body>
              <#if orderHeader.orderTypeId == "PURCHASE_ORDER" && groupedItems?exists>
                <#include "orderItemsGroupedByProduct.fo.ftl"/>
              <#else>
                 ...
                 <#if (note.internalNote?has_content) && (note.internalNote != "Y")>
                    ...
                    <fo:table-row>
                      ...
                      <fo:table-cell number-columns-spanned="2">
                        <fo:block>${uiLabelMap.CommonAt}: <@displayDateFO date=note.noteDateTime format="DATE_TIME"/></fo:block>    
                      </fo:table-cell>
                    </fo:table-row>

                    <fo:table-row>
                      <fo:table-cell number-columns-spanned="5">
                        <fo:block font-size="10pt">${note.noteInfo?if_exists}</fo:block>    
                      </fo:table-cell>
                    </fo:table-row>
                  </#if>                  
                  </#list>
