...
{	
    ...
    public Iterable<String> getAllKeys()
    {
        ...
        for(GenericValue party : _allParties) { keys.add(party.getString("partyId")); }
    ...
    {
        for(GenericValue party : EntitySyncSourceHelper.getNewEntities(_allParties, since)) { _newKeys.add(party.getString("partyId")); }
    ...
    {
        Set<String> keys = new HashSet<String>();
        for(GenericValue party : EntitySyncSourceHelper.getUpdatedParties(_allParties, since)) { keys.add(party.getString("partyId")); }
    ...
    public Iterable<String> getDeletedKeys(Timestamp since) throws GenericEntityException
    {
        ...
        EntityExpr sinceExpr = new EntityExpr("deactivationTimestamp", EntityOperator.GREATER_THAN, since);
        for(GenericValue deleted : _syncSource.findByAnd("PartyDeactivation", sinceExpr))
        {
        ...
        if(!StringUtils.isBlank(fullName))  //if we have a name, it could be an CONTACT or LEAD
        {
            for(GenericValue candidate : _allParties)
            {
                ...
                if("CONTACT".equals(roleType) || "PROSPECT".equals(roleType))
                {
                    ...
                    if(fullName.equals(candidateFullName)) { twinKeys.add(candidate.getString("partyId")); }
                }
            }
        }
        else  //2. failing that, look by company - in this case only ACCOUNTS are interesting
        {
            ...
            for(GenericValue candidate : _allParties)
            {
                ...
                if("ACCOUNT".equals(roleType))
                {
                    ...
                    if((groupName != null) && groupName.equals(candidateGroupName)) { twinKeys.add(candidate.getString("partyId")); }
                }
            }        
        }
        return twinKeys;
     ...
     * @param key the partyId of the Party to "delete"
     * @param since
     */
    ...
    /** 
     * @param key should be partyId of an Account, Contact or Lead
     * @param source should be a Contact
     */
    ...
    {    	
        ...
        if(UtilValidate.isEmpty(roleType)) { throw new GenericServiceException("Could not find any service to UPDATE party " + partyId); }
    ...
    {
    	if(isContactFieldEmpty(contact, purpose)) { return; } 
        ...
        throws GenericEntityException, GenericServiceException
    {
        ...
        if(records.size() == 0) { return recordChanged; }
        ...
        for(EntityBeanConverter addressConverter : _converters.listConverters("PostalAddress", contact))
        {
            ...
            if(purpose != null)  //override default empty values with actual data
            {
                ...
                if(postalAddress!=null)
                {
                    ...
                    if(!StringUtils.isEmpty(street2)) { street += "\r\n" + street2; }           
                }            
            }
    ...
    private void prepareContactTypifiedProperty(GenericValue purpose, Contact contact, String type) throws GenericEntityException
    {
        ...
        if(purpose!= null)  //we actually have something to update
        {
            ...
            if(infoStr != null) { infoString = infoStr; }
        }
    ...
    private GenericValue getActivePartyContactMechPurpose(GenericValue contactParty, String purpose) throws GenericEntityException
    {
        ...
        if(activePCMs.size() == 0) { return null; }  //there is nothing
        ...
        throws GenericEntityException, GenericServiceException
    {
        ...
        if(partyContactMech != null)  //we actually have something to update!
        {
            ...
            if(isEmailPurpose(purpose)) { serviceParams.put("contactMechTypeId", mechType); }
              
            serviceParams.put("partyId", partyContactMech.getString("partyId"));        

            EntityBeanConverter converter = null;
            if(service.getEntity() == null) { converter = _converters.getConverter(purpose,contact); }
            else { converter = _converters.getConverter(service.getEntity(), contact, purpose); }
            converter.toMap(contact, serviceParams);  //set other params
    ...
    {
    	if(purpose.contains("EMAIL")){return true;}
    	return false;
    }
    ...
    private void buildPhone(List<TypifiedProperty> phones, GenericValue contactParty, String phonePurpose, String phoneType) throws GenericEntityException
    {
        ...
        if(phoneGV != null)
        {
            ...
            { 
                ...
                if(UtilValidate.isEmpty(contact)) { contact="";  }
                
                String extension = phoneGV.getString("extension");  //bizarrely, extension is store in the PCM
                if(UtilValidate.isEmpty(extension)) { extension = "";  }
                else { extension = " " + extension; } //convert to outlook format
                
                //finally concatenate all the parts
                phoneString = country + area + contact; // + extension;
            } 
            else { log.warn(phonePurpose +  " without TelecomNumber"); }
        }
    ...
    private void addressCompatibilization(Contact contact) throws GenericEntityException
    {
    	...
    	for(Address address : addresses)
    	{
	    	...
	    	if(!UtilValidate.isEmpty(country))
	    	{
                ...
                if(countryId == null) { countryId = _defaultCountryGeoId; }  //we can use default value
	    	...
	    	if(!UtilValidate.isEmpty(state))  //1.2.1 try a reverse lookup
	    	{
                ...
                for(String fieldToCheck : new String[] { "geoId", "geoCode", "abbreviation", "geoName" })
                {
                    ...
                    if(stateId != null) { break; }  //found something!
                }
	    	}
            if(stateId == null)  //if we still have nothing, use a default
            { 
                if("USA".equals(countryId)) { stateId = _defaultStateGeoId_US; }
                else { stateId = ""; }  //there is no logical default value here
            }
            ...
            if(streets != null && streets.length > 0)
            {
                ...
                if(streets.length > 1) { address.getExtendedAddress().setPropertyValue(streets[1]); }
            }
            ...
            if(StringUtils.isBlank(postcode.getPropertyValueAsString())) { postcode.setPropertyValue(_emptyFieldString); }
            
            //1.5 city (some OT services do not allow it to be empty)
            Property city = address.getCity();
            if(StringUtils.isBlank(city.getPropertyValueAsString())) { city.setPropertyValue(_emptyFieldString); }
    	}    	
    }
    ...
    {
        List<EntityExpr> criteria = new ArrayList<EntityExpr>();
        criteria.add(new EntityExpr(geoField, EntityOperator.EQUALS, value));
        
        if(geoType!=null)
        {
            criteria.add(new EntityExpr("geoTypeId", EntityOperator.EQUALS, geoType));
        }
