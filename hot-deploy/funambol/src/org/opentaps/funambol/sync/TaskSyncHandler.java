...
{ 
    ...
    private static enum EventState {EVENT_SCHEDULED,EVENT_STARTED,EVENT_COMPLETED,EVENT_ON_HOLD,EVENT_CANCELLED}
    private static enum TaskState {TASK_SCHEDULED,TASK_STARTED,TASK_COMPLETED,TASK_ON_HOLD,TASK_CANCELLED} 
    ...
    { 
         ...
         * 
         *  @throw NotConfiguredException if no corresponding state found
         */
        public String stateFor(String statusId)
        {
           ...
           {
               status++;
               if(state.toString().equals(statusId)) { return String.valueOf(status); }
           }
         ...
         */
        public String firstStatus() { return _states.iterator().next().toString(); }
        ...
        public String statusFor(int status)
        {
            ...
            Object state = null;
            for(int i = 0; i <= status; i++) { state = allStates.next(); }
        ...
        { 
            try
            {
                if(dateStr.charAt(dateStr.length() - 1) == 'Z') { return DATE_TIME_FORMAT.parse(dateStr); }
                else { return DATE_ONLY_FORMAT.parse(dateStr); }
            }
            catch (ParseException e) { throw new RuntimeException("Cannot parse date from " + dateStr); } //TODO: what to throw here
        }
    ...
    public Iterable<String> getAllKeys() 
    {
        ...
        for(GenericValue party : _allTasks) { keys.add(party.getString("workEffortId")); }
        
        return keys;
    }

    public Iterable<String> getDeletedKeys(Timestamp since)
    throws GeneralException 
    {
        Set<String> deletedKeys = new HashSet<String>();  //use a set in case any Id comes up twice
        EntityExpr sinceExpr = new EntityExpr("thruDate", EntityOperator.GREATER_THAN_EQUAL_TO, since);
        for( GenericValue deleted : _syncSource.findByAnd("WorkEffortPartyAssignment", sinceExpr) )
        {
    ...
    {
        if( inputValue == null || tag == null) return null;
        Object propVal = null; 
    ...
    public Iterable<String> getNewKeys(Timestamp since) throws GenericEntityException 
    {
        ...
        { 
            ...
            if(EntitySyncSourceHelper.wasCreated(workEffort, since)) { keys.add(weId); }
    ...
    public byte[] convertBeanToData(CalendarContent item) throws GeneralException 
    {
        try 
        {
            ...
            if(item instanceof Task) { cal.setTask((Task)item); }
            else if(item instanceof Event) { cal.setEvent((Event)item); }
            else { throw new NotConfiguredException("Cannot convert unrecognized item " + item + " to raw data"); }
    ...
    private String calendar2sif(Calendar calendar, String sifType) throws EntityException 
    {
        ...
        {            
            ...
            if(sifType.equals(Type.EVENTSIF.getMimeType()))
            { // SIF-E
                ...
                thing = calendar;  //TODO: should it not be Calendar.getEvent()?
                    ...
                    else if(sifType.equals(Type.TASKSIF.getMimeType()))
            { // SIF-T
        ...
        catch(Exception e) { throw new EntityException("Error converting Calendar to " + sifType, e); }
    }
    ...
    private String calendar2cal(Calendar calendar, String calType) throws EntityException 
    {
        ...
        catch(Exception e) { throw new EntityException("Error converting Calendar to " + calType, e); }
    }
    ...
    private List<GenericValue> prepareAllTasks() throws GeneralException
    {
        ...
        criteria.add(new EntityExpr("partyId", EntityOperator.EQUALS, _syncPartyId));
        criteria.add(new EntityExpr("roleTypeId", EntityOperator.IN, UtilMisc.toList("CAL_OWNER", "CAL_ATTENDEE")));
