...
public class OpentapsFedexServices {
    ...
    static {
        try {
            fedexSoapUrl = new URL(UtilProperties.getPropertyValue(shipmentPropertiesFile, "shipment.fedex.connect.soap.url"));
        } catch (MalformedURLException e) {
            Debug.logError("shipment.fedex.connect.soap.url is not configured in " + shipmentPropertiesFile + " - defaulting to test URL at https://gatewaybeta.fedex.com:443/web-services", module);
            try {
    ...
    public static Map fedexRateRequest(DispatchContext dctx, Map context) {
        ...
        if (UtilValidate.isEmpty(weightUomId)) {
            ...
            return ServiceUtil.returnFailure("WeightUomId in " + shipmentPropertiesFile + ":shipment.default.weight.uom must be either WT_lb or WT_kg.");
        }
        ...
        if (shippableWeight.doubleValue() == 0) {
            try {
                shippableWeight = Double.valueOf(UtilProperties.getPropertyValue(shipmentPropertiesFile, "shipment.default.weight.value"));
        ...
        try {
            ...
            if (shippingOriginContactMechId != null) {
                ...
                if ((productStore != null) &&
                    ...
                    if (UtilValidate.isNotEmpty(shipLocs)) {
                        ...
                        if (purp != null) {
                            shipFromAddress = delegator.findByPrimaryKey("PostalAddress",
                                    UtilMisc.toMap("contactMechId",
                                        purp.getString("contactMechId")));

                            if (shipFromAddress != null) {
                                originationZip = shipFromAddress.getString(
                                        "postalCode");
                            }
                        }
                    }
            ...
            if (UtilValidate.isEmpty(originationZip)) {
                return ServiceUtil.returnFailure(
                    "Unable to determine the origination ZIP");
            }
            // retrieving the countryCode
            List countryCodes = Collections.emptyList();
            ...
            if(UtilValidate.isNotEmpty(countryCodes)) {
                ...
                return ServiceUtil.returnFailure(
                "Unable to determine the origin country");
            }

            // get the destination ZIP
            String destinationZip = null;
            String shippingContactMechId = (String) context.get(
            ...
            if (UtilValidate.isEmpty(destinationZip)) {
                return ServiceUtil.returnFailure(
                    "Unable to determine the destination ZIP");
            }
            ...
            if(UtilValidate.isNotEmpty(countryCodes)) {
                ...
                return ServiceUtil.returnFailure(
                "Unable to determine the destination country");
            }
            ...
            if (UtilValidate.isNotEmpty(shipToAddress.getString("address2"))) streetLines.add(shipToAddress.getString("address2"));
            destAddress.setStreetLines(streetLines.toArray(new String[]{}));
            destAddress.setCity(shipToAddress.getString("city"));
            if (UtilValidate.isNotEmpty(shipToAddress.getString("stateProvinceGeoId"))) destAddress.setStateOrProvinceCode(delegator.findByPrimaryKeyCache("Geo", UtilMisc.toMap("geoId", shipToAddress.getString("stateProvinceGeoId"))).getString("geoCode"));
            ...
            if (UtilValidate.isNotEmpty(shipFromAddress.getString("address2"))) streetLines.add(shipFromAddress.getString("address2"));
            originAddress.setStreetLines(streetLines.toArray(new String[]{}));
            originAddress.setCity(shipFromAddress.getString("city"));
            if (UtilValidate.isNotEmpty(shipFromAddress.getString("stateProvinceGeoId"))) originAddress.setStateOrProvinceCode(delegator.findByPrimaryKeyCache("Geo", UtilMisc.toMap("geoId", shipFromAddress.getString("stateProvinceGeoId"))).getString("geoCode"));
            ...
            if (UtilValidate.isEmpty(shippableQuantity)) shippableQuantity = new Double(0);
            Weight weight = new Weight(WeightUnits.fromString(weightType), new BigDecimal(shippableWeight.doubleValue()).setScale(1, BigDecimal.ROUND_UP).doubleValue());
            Money totalAmount = new Money(currency, new BigDecimal((Double) context.get("shippableTotal")).setScale(2, BigDecimal.ROUND_HALF_UP));
            ...
            rateRequest.setDropoffType(getDropoffType(UtilProperties.getPropertyValue(shipmentPropertiesFile, DROPOFF_TYPE)));
            rateRequest.setPackagingType(getPackagingType(UtilProperties.getPropertyValue(shipmentPropertiesFile, PACKAGING_TYPE)));
            ...
            if (reply != null) {
                ...
                if ("SUCCESS".equalsIgnoreCase(notifType.toString())) {
                    ...
                    if (details.length > 0) {
                        ...
                        if (totalNetCharge != null) shippingEstimateAmount = totalNetCharge.getAmount();
                    }
                } else {
